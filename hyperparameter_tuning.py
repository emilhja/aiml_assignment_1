...
def create_tuning_notebook(tuning_root, run_specs):
    ...
    notebook.cells = [
        ...
        _markdown_cell(
            "## Final Summary\n\n"
            "The newest tuning sweep changes the earlier conclusion. The best overall run is now "
            "`tune_14_wide_3conv`, a wider 3-convolution model (`48 -> 96 -> 128`) with batch "
            "normalization, `0.2` dropout, light weight decay, and small input noise. This run "
            "achieved the highest test accuracy in the sweep (`0.9939`) while also keeping runtime "
            "reasonable. That suggests that widening the deeper CNN was more useful than only "
            "augmenting the previous smaller architecture.\n\n"
            "At the same time, the best validation metrics came from other settings. "
            "`tune_10_deep_augmented` achieved the best validation accuracy (`0.9927`) and best "
            "validation loss among the strongest test runs (`0.02685`), while `tune_05_noise` also "
            "performed very well on validation loss (`0.02705`) but was weaker on final test "
            "accuracy. This pattern suggests that augmentation and noise injection improve training "
            "stability and validation behavior, but the best final test performance came from a "
            "carefully balanced architecture with moderate regularization.\n\n"
            "The extended sweep also made the hyperparameter effects clearer. A larger hidden layer "
            "(`512`) did not improve test accuracy enough to justify its large parameter increase, "
            "while a smaller hidden layer (`64`) underperformed. A LeakyReLU version of the deeper "
            "batch-normalized model stayed competitive, but did not beat the best ReLU runs. The "
            "Adam epsilon variant improved over the baseline batchnorm model, whereas the Adam beta "
            "variant did not become a top performer. Overall conclusion: the best result came from "
            "combining a wider 3-layer CNN with batch normalization, moderate dropout, light weight "
            "decay, and a small amount of input noise."
            "\n\n"
            "### Kort Svensk Sammanfattning\n\n"
            "Den senaste hyperparameter-tuningen ändrar den tidigare slutsatsen. Den bästa totala "
            "körningen är nu `tune_14_wide_3conv`, en bredare 3-conv-modell (`48 -> 96 -> 128`) "
            "med batch normalization, `0.2` dropout, lätt weight decay och lite input noise. Denna "
            "modell gav högst test accuracy i sweepen (`0.9939`) och hade samtidigt rimlig "
            "träningstid. Det tyder på att en bredare djup CNN gav större effekt än att bara lägga "
            "till augmentation på den mindre modellen.\n\n"
            "Samtidigt kom de bästa validation-måtten från andra inställningar. "
            "`tune_10_deep_augmented` gav bäst validation accuracy (`0.9927`) och bäst validation "
            "loss bland de starkaste testkörningarna (`0.02685`), medan `tune_05_noise` också gav "
            "mycket bra validation loss (`0.02705`) men svagare test accuracy. Det tyder på att "
            "augmentation och noise injection förbättrar träningsstabilitet och validation-resultat, "
            "men att bästa testresultatet kom från en välbalanserad arkitektur med måttlig "
            "regularisering.\n\n"
            "Den utökade sweepen gjorde också hyperparameter-effekterna tydligare. Ett större hidden "
            "layer (`512`) förbättrade inte resultatet tillräckligt för att motivera det mycket större "
            "antalet parametrar, medan ett mindre hidden layer (`64`) blev sämre. En LeakyReLU-version "
            "av den djupare batchnorm-modellen var konkurrenskraftig men slog inte de bästa ReLU-runsen. "
            "Adam epsilon-varianten förbättrade resultatet jämfört med batchnorm-baseline, medan Adam "
            "beta-varianten inte blev en toppmodell. Slutsats: bästa resultatet kom från en bredare "
            "3-lagers CNN med batch normalization, måttlig dropout, lätt weight decay och lite input noise."
        ),
    ]
