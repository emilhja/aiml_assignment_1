...
def create_comparison_notebook(comparison_root, summaries):
    ...
    notebook.cells = [
        ...
        _markdown_cell(
            "## Final Summary\n\n"
            "The newest regularization comparison again shows that weight decay is the most "
            "effective single regularization method in this experiment. The "
            "`weight_decay_only` run achieved the best test accuracy (`0.9922`), while the plain "
            "baseline was almost identical at `0.9921` and also slightly faster. This means that "
            "weight decay provided a small but consistent generalization benefit without adding "
            "much runtime cost.\n\n"
            "Dropout and batch normalization both produced stable models, but neither improved "
            "test accuracy over the baseline in this run. The combined setup with dropout, batch "
            "normalization, weight decay, and input noise achieved the best validation loss "
            "(`0.03260`), but its test accuracy was lower (`0.9907`) and runtime was longer. "
            "That suggests the combined model was more strongly regularized, but that the extra "
            "regularization did not translate into better final test-set performance.\n\n"
            "Overall conclusion: for this MNIST task, moderate regularization worked best. "
            "Weight decay was the strongest standalone choice, while stacking several "
            "regularization methods at once improved validation loss but did not improve the "
            "final test result."
            "\n\n"
            "### Kort Svensk Sammanfattning\n\n"
            "Den senaste regulariseringsjämförelsen visar återigen att weight decay var den "
            "mest effektiva enskilda regulariseringsmetoden. Körningen `weight_decay_only` gav "
            "högst test accuracy (`0.9922`), medan baseline låg nästan lika högt (`0.9921`) och "
            "var något snabbare. Det betyder att weight decay gav en liten men tydlig förbättring "
            "i generalisering utan någon större extra kostnad i träningstid.\n\n"
            "Dropout och batch normalization gav stabila modeller, men ingen av dem slog "
            "baseline i test accuracy i denna körning. Den kombinerade setupen med dropout, "
            "batch normalization, weight decay och input noise gav bäst validation loss "
            "(`0.03260`), men sämre test accuracy (`0.9907`) och längre träningstid. Det tyder "
            "på att modellen blev mer regulariserad, men att detta inte gav bättre slutresultat "
            "på testdata.\n\n"
            "Slutsats: för denna MNIST-uppgift fungerade måttlig regularisering bäst. Weight "
            "decay var det starkaste enskilda valet, medan flera regulariseringsmetoder samtidigt "
            "förbättrade validation loss men inte den slutliga test accuracy."
        ),
    ]
